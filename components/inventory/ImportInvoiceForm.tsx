...
export type ImportInvoiceFormProps = {
  /** Shown when embedded in a modal — adds a "Done" affordance on the success screen. */
  onClose?: () => void;
  /** If provided, the success-screen "View purchases" action calls this (e.g. to swap modals). */
  onViewPurchases?: () => void;
  /** Tighten the PDF preview height for modal embedding. */
  compact?: boolean;
};
...
/**
 * Invoice import flow: PDF upload + OCR auto-extract + line entry + submit.
 * Designed to be embedded either standalone (full page) or inside a modal.
 * No auth guard or PageHeader — the host is responsible for those.
 */
export function ImportInvoiceForm({
  ...
  if (summary) {
    return (
      <Card className={compact ? "max-w-md mx-auto" : "max-w-md"}>
        ...
        <CardBody>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            {onViewPurchases ? (
              <Button variant="secondary" onClick={onViewPurchases} leftIcon={<Icon name="truck" size={16} />}>
                View purchases
              </Button>
            ) : (
              <Link
  ...
  const previewHeight = compact ? "h-[55vh]" : "h-[70vh]";
        ...
        <CardBody className="space-y-3">
          ...
          <div className="overflow-hidden rounded-lg border border-border bg-surface-2">
            {objectUrl ? (
              <iframe
                src={objectUrl}
                className={`${previewHeight} w-full`}
                title="Invoice PDF"
              />
            ) : (
              <div className={`flex ${previewHeight} items-center justify-center`}>
                <EmptyState
                  icon="receipt"
...
/** Loading placeholder used by the standalone page while currentUser resolves. */
export function ImportInvoiceFormSkeleton({ compact = false }: { compact?: boolean }) {
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <Card>
        <CardBody className="space-y-3">
          ...
          <Skeleton height={40} />
          <Skeleton height={compact ? "55vh" : "60vh"} />
        </CardBody>
      </Card>
      <Card>
        <CardBody className="space-y-3">
